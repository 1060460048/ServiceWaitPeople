...
void Worker::doWork()
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        QEventLoop loop;
        QTimer::singleShot(100*(qrand()%((10))+1), &loop, SLOT(quit()));
        loop.exec();


        qDebug()<<id<<": is not busy now ,plea next client  ...";
        emit notbusy (id);

//        qsrand(qrand());
//        Mat img(Size(320,240), CV_8UC3,cv::Scalar(qrand()%((i + 1)), qrand()%((i + 1)), qrand()%((i + 1))));
